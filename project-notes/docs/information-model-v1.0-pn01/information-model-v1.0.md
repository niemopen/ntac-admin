...
3. **XSD as a data model formalism**: the JADN information model formalism captures the XSD constraints
needed for applications to satisfy their requirements but does not necessarily replicate the
data objects defined by XSD. JADN can support both multiple "styles" of XML and type-specific serialization
options, but the latter is contrary to the goals of abstraction. Where byte-for-byte backward compatibility
